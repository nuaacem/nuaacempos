...
public class HomeController {  
    ...
    public String index(HttpServletRequest request, Model model) {
    	...
    	if (!model.containsAttribute("feedList")) {
    		List<TFeed> tFeeds = feedService.findBaseTime(100);
    		List<FeedShowModel> feedShowModels = new ArrayList<FeedShowModel>();
    		for (TFeed tFeed : tFeeds) {
				feedShowModels.add(FeedModelExtension.toFeedShowModel(tFeed));
			}
    		model.addAttribute("feedList", feedShowModels);
    	}
