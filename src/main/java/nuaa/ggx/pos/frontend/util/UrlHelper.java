package nuaa.ggx.pos.frontend.util;
...
public class UrlHelper {
	...
	public static String explainWebsiteUrl(String websiteUrl) {
		if (StringUtils.startsWith(websiteUrl, "http://")) {
			websiteUrl = StringUtils.substring(websiteUrl, 7);
		}
		int first = StringUtils.indexOf(websiteUrl , "/");
		if (first > 0) {
			int second = StringUtils.indexOf(websiteUrl, "/", first + 1);
			if (second > 0) {
				return StringUtils.substring(websiteUrl, 0, second + 1);
			}
		}
		else {
	...
	public static void main(String[] args) {
		System.out.println(explainWebsiteUrl("http://www.sina.com.cn/a"));
	}
}
